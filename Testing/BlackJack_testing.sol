...
contract BlackJack_testing {
    ...
    mapping(address => uint256[156]) public mapInt1;
    mapping(address => uint256[156]) public mapInt2;
    ...
    function getGamestateByKey(address _user) public view returns (GameState) {
        return mapGamestate[_user];
    }

    function getPlayerCardByKey(address _user) public view returns (uint256[12] memory) {
        return mapPlayer_card[_user];
    }

    function getCasinoCardByKey(address _user) public view returns (uint256[12] memory) {
        return mapCasino_card[_user];
    }

    function getGameResultByKey(address _user) public view returns (GameResult) {
        return mapGameResult[_user];
    }


    function getPlayerAceCount(address _user) public view returns (uint256){
        return player_AceCount[_user];
    }

    function getCasinoAceCount(address _user) public view returns (uint256){
        return casino_AceCount[_user];
    }

    function getMapGameDeckindexByKey(address _user) public view returns (uint256){
        return mapGameDeckindex[_user];
    }

    function getPlayerCardNumber(address _user) public view returns (uint256){
        return mapPlayer_card_num[_user];
    }

    function getCasinoCardNumber(address _user) public view returns (uint256){
        return mapCasino_card_num[_user];
    }

    function getMapGameDeckByKey(address _user) public view returns (uint256[52] memory){
        return mapGameDeck[_user];
    }
    function getAddressKey() public view returns (address[] memory){
        return addressKeys;
    }


    function setDeck(address _user, uint256 first_card, uint256 second_card, uint256 third_card, uint256 fourth_card, uint256 fifth_card) public {
       mapGameDeck[_user][0] = first_card;
        mapGameDeck[_user][1] = second_card;
        mapGameDeck[_user][2] = third_card;
        mapGameDeck[_user][3] = fourth_card;
        mapGameDeck[_user][4] = fifth_card;
    }
