       ...
       return (
              <div className={styles.dealerHandContainer}>
                     {(deck.length != 0)?<div className={styles.title}>
                            {(Props.revealPhase)?<div>Dealer's hand: {total} </div>: <div>Dealer's hand: ??? </div>}
                     </div>:null}
                     <div className={styles.cards}>
       {deck? deck.map((card, index)=> (<div className={styles.innerCards}><Card number={card.value} suit={card.suit} color={card.color}/> {(Props.revealPhase)?null:<Card/>}</div>)): null}
                     </div>
              </div>
       )
}
